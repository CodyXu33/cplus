...
### 23.1 先把名词说清楚
- 内存管理：对象什么时候创建、在哪里创建、什么时候释放。
- 栈内存（stack）：由作用域自动管理，进入作用域构造，离开作用域析构。
- 堆内存（heap/free store）：手动申请释放（或通过 RAII 封装自动释放）。
- 生命周期（lifetime）：对象从构造到析构的有效时间区间。
- RAII：资源获取即初始化。把资源绑定到对象生命周期。

一句话：C++ 难点不是“能申请内存”，而是“能稳定地回收资源”。
...
```cpp
void f() {
    int a = 1;        // 栈对象：函数结束自动销毁
    int* p = new int(2); // 堆对象：需要 delete
    delete p;
}
```

### 23.3 生命周期与作用域不是一回事
- 大多数局部变量：作用域结束即生命周期结束。
- 通过 `new` 申请的对象：即使指针变量离开作用域，对象也可能还活着（若没 delete 就泄漏）。

```mermaid
flowchart TD
  A[变量作用域结束] --> B{对象在哪分配?}
  B -- 栈 --> C[自动析构]
  B -- 堆 --> D[对象仍存在, 需显式释放或 RAII]
```

### 23.4 `new/delete` 与 `new[]/delete[]` 配对规则
必须严格配对：
- `new` 对应 `delete`
- `new[]` 对应 `delete[]`
...
配错是未定义行为，可能崩溃也可能静默出错。

### 23.5 经典内存错误类型
- 内存泄漏（leak）：申请后未释放。
- 悬空指针（dangling）：指向已释放对象。
- 重复释放（double free）：同一资源释放两次。
- 越界访问（out-of-bounds）：访问分配范围外内存。
- 野指针（wild pointer）：未初始化指针被使用。
...
class FileReader {
public:
    explicit FileReader(const std::string& path) : in(path) {
        if (!in.is_open()) {
            throw std::runtime_error("open file failed");
        }
    }

    ~FileReader() = default; // ifstream 自己会关闭文件
...
重点：现代 C++ 倡导“让成员本身就是 RAII 类型”（Rule of Zero）。

### 23.7 手写资源类的最小原则
如果必须管理裸资源（如 C API 句柄）：
- 明确所有权（谁负责释放）
- 明确不可拷贝或深拷贝策略
- 支持移动语义
- 析构保证释放且不抛异常

这就是为什么后面要学智能指针：减少手写资源类。

### 23.8 资源不止是内存
RAII 管的不只是 `new/delete`：
- 文件句柄
- socket 连接
- 互斥锁
- 数据库连接
...
### 23.9 异常场景下为什么 RAII 更关键
如果中途抛异常，手写 `delete/close/unlock` 很容易漏。  
RAII 依赖析构自动清理，天然和异常机制配合。

```mermaid
sequenceDiagram
  participant F as 函数
  participant R as RAII对象
  F->>R: 构造并获取资源
  F-->>F: 发生异常
  R-->>F: 析构自动释放资源
```
...
### 23.10 栈 vs 堆：实战决策
- 能用栈对象就先用栈对象。
- 需要跨作用域共享/延迟释放时，再考虑堆对象（优先智能指针）。
- 避免在业务代码直接写大量裸 `new/delete`。

工程里“少手动管理”通常比“手动管理得很炫”更可靠。

### 23.11 常见错误模式
- 把 `new` 当默认创建方式，导致代码复杂且易泄漏。
- `new[]` 后用 `delete`（或反过来）。
- 释放后不置空，后续误用悬空指针。
- 资源管理分散在多个分支里，异常路径忘记清理。
- 把“临时可用”当“长期可维护”，忽视 RAII 封装。

### 23.12 第 23 小节知识总结
- 内存管理本质是生命周期管理，核心问题是“谁拥有、谁释放”。
- 栈对象是默认首选；堆对象需要明确所有权策略。
- RAII 是现代 C++ 资源管理基石，能显著降低泄漏和异常路径错误。
- 下一节智能指针就是 RAII 在堆内存管理里的标准方案。
